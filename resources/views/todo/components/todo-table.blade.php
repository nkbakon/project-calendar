<div class="overflow-x-auto">
    <div class="flex w-full p-3 space-x-2">
        ...
        <div class="w-3/6">
            ...
            focus:outline-none focus:shadow-outline focus-ring-0 foucus:bg-indigo-50" placeholder="Search To Do...">
        </div>
        <!-- Order By -->
        <div class="w-1/6">
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">                                    
                ...
                <td class="hidden">
                    ...
                    @for ($i = 0; $i < count($department_ids); $i++)
                        {{ App\Models\Department::find($department_ids[$i])->name }},
                    @endfor                                   
                </td>
                <td class="py-3 px-4">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium leading-4 bg-{{ $todo->status_color }}-100 text-{{ $todo->status_color }}-900 capitalize">
                        {{ $todo->status }}   
                    </span>                                                            
                </td>
                <td class="hidden">
...
<div id="viewData" tabindex="-1" class="bg-opacity-50 fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6">
                <h3 class="mb-5 text-center text-xl font-normal text-gray-700 dark:text-gray-400">To Do</span></h3>
                <div>
                    <strong>ID : </strong><span id="view_id"></span><br>
                    <strong>Title : </strong><span id="view_title"></span><br>
                    <strong>Departments : </strong><br><span id="view_departments"></span>
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 text-center">
                <form method="POST" action="{{ route('todo.destroy', 'data_id') }}">
                    @csrf
                    @method('DELETE')
...
<script>
    ...
    $(document).ready(function(){
        $('.viewBtn').on('click', function(){
            ...
            document.getElementById('view_title').innerHTML = data[1];
            document.getElementById('view_departments').innerHTML = data[3].replaceAll(',', '<br>');
            document.getElementById('view_status').innerHTML = data[4];
            document.getElementById('view_addby').innerHTML = data[5];
            document.getElementById('view_adddate').innerHTML = data[6];
            document.getElementById('view_editby').innerHTML = data[7];
            document.getElementById('view_editdate').innerHTML = data[8];
        });
    });
</script>
@endpush